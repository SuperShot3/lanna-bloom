...
import {
  ORDER_ITEM_DELIVERY_PHOTO_BUCKET,
  ORDER_ITEM_DELIVERY_PHOTO_SIGNED_TTL_SECONDS,
  buildOrderItemDeliveryPhotoStoragePath,
  createOrderItemDeliveryPhotoSignedUrl,
  getOrderItemForDeliveryPhoto,
  isOrderItemDeliveryPhotoStoragePath,
} from '@/lib/admin/itemDeliveryPhoto';
...
async function removeStoragePath(path: string): Promise<void> {
  ...
  if (!supabase) return;
  await supabase.storage.from(ORDER_ITEM_DELIVERY_PHOTO_BUCKET).remove([path]);
}
...
export async function GET(
  ...
  const found = await getOrderItemForDeliveryPhoto(orderId, itemId);
  if (!found.ok) {
    return NextResponse.json({ error: found.error }, { status: found.status });
  }
  const path = found.item.delivery_photo_path?.trim();
  if (!path) {
    return NextResponse.json({ error: 'No delivery photo attached' }, { status: 404 });
  }
  if (!isOrderItemDeliveryPhotoStoragePath(orderId, itemId, path)) {
  ...
  const signed = await createOrderItemDeliveryPhotoSignedUrl(path, shouldDownload);
  if (!signed.ok) {
  ...
  return NextResponse.json({
    ...
    path,
    expiresIn: ORDER_ITEM_DELIVERY_PHOTO_SIGNED_TTL_SECONDS,
  });
}
...
export async function POST(
  ...
  const storagePath = buildOrderItemDeliveryPhotoStoragePath(orderId, itemId, ext);
  ...
  if (uploadError) {
    console.error('[item-delivery-photo] upload error:', uploadError.message);
    return NextResponse.json({ error: 'Upload failed' }, { status: 500 });
  }

  const previous = found.item.delivery_photo_path?.trim() || null;
  ...
  if (
    ...
    isOrderItemDeliveryPhotoStoragePath(orderId, itemId, previous)
  ) {
    await removeStoragePath(previous);
  }

  const signed = await createOrderItemDeliveryPhotoSignedUrl(storagePath);
  if (!signed.ok) {
    return NextResponse.json({
      path: storagePath,
      signedUrl: null,
...
export async function DELETE(
  ...
  const { error: updateError } = await supabase
    .from('order_items')
    .update({ delivery_photo_path: null })
    .eq('order_id', orderId)
    .eq('id', itemId);
  if (updateError) {
    return NextResponse.json({ error: updateError.message }, { status: 500 });
  ...
  if (previous && isOrderItemDeliveryPhotoStoragePath(orderId, itemId, previous)) {
    await removeStoragePath(previous);
  }
