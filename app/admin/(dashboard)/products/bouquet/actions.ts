...
export async function updateBouquetByAdminAction(formData: FormData): Promise<{ error?: string }> {
  ...
  const input: UpdateCatalogBouquetByAdminInput = {
    ...
    titleIntroEn: String(formData.get('titleIntroEn') ?? ''),
    titleIntroTh: String(formData.get('titleIntroTh') ?? ''),
    floristNoteEn: String(formData.get('floristNoteEn') ?? ''),
    floristNoteTh: String(formData.get('floristNoteTh') ?? ''),
    ...
    if (bouquetUsesDraftWorkflow(detail.status)) {
      await saveCatalogBouquetDraft({
        ...
        payload: {
          ...
          titleIntroEn: input.titleIntroEn,
          titleIntroTh: input.titleIntroTh,
          floristNoteEn: input.floristNoteEn,
          floristNoteTh: input.floristNoteTh,
...
export async function convertBouquetImageToWebpAction(formData: FormData): Promise<{ error?: string }> {
  const session = await auth();
  if (!session?.user) return { error: 'Unauthorized - Session not found' };
  if (!canChangeStatus((session.user as { role?: string }).role)) {
    return { error: 'Forbidden' };
  ...
  try {
    ...
    console.error('[Products] convertBouquetImageToWebp failed:', err);
