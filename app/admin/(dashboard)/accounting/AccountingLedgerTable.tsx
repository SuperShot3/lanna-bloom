...
export function AccountingLedgerTable({ ledger, periodLabel }: Props) {
  ...
  const filteredRows = useMemo(() => {
    ...
    if (subView === 'withdrawal') return all.filter((r) => r.kind === 'withdrawal');
    if (subView === 'missing') {
      return all.filter(
  ...
  const exportCurrentViewCsv = () => {
    ...
    const isWithdrawal = subView === 'withdrawal';
    ...
    if (isWithdrawal) headers.push('From');
    if (isIncome || isAll) headers.push('Amount in');
    if (isExpense || isAll || isMissing || isWithdrawal) headers.push('Amount out');
    ...
    for (const r of allRows) {
      ...
      if (isTransfer || isWithdrawal) cells.push(escapeCsvCell(r.sourceAccount));
      if (isIncome || isAll) cells.push(r.amountIn != null ? String(r.amountIn) : '');
      if (isExpense || isAll || isMissing || isWithdrawal) {
        cells.push(r.amountOut != null ? String(r.amountOut) : '');
      }
    ...
    <div className="admin-accounting-section admin-ledger">
      ...
      <div className="admin-ledger-subtabs" role="tablist" aria-label="Ledger view">
        ...
        <SubTabButton
          ...
          value="withdrawal"
          label="Withdrawals"
          count={ledger.counts.withdrawal}
          onSelect={setSubView}
        />
        <SubTabButton
          current={subView}
...
function Thead({ subView, showRunningBalance }: { subView: SubView; showRunningBalance: boolean }) {
  ...
  const showSource = subView !== 'transfer' && subView !== 'withdrawal';
  const showFromTo = subView === 'transfer';
  const showFrom = subView === 'withdrawal';
  const showIn = subView === 'all' || subView === 'income';
  const showOut = subView === 'all' || subView === 'expense' || subView === 'missing' || subView === 'withdrawal';
  const showReceipt =
    subView === 'all' || subView === 'income' || subView === 'expense' || subView === 'missing' || subView === 'withdrawal';
...
function LedgerTr({
  ...
  return (
    <tr
      className={
        r.kind === 'income'
          ...
          : r.kind === 'transfer' || r.kind === 'withdrawal'
            ? 'admin-ledger-row'
    ...
    >
      ...
      {showFrom && <td className="admin-ledger-nowrap">{r.sourceAccount}</td>}
      {showIn && (
