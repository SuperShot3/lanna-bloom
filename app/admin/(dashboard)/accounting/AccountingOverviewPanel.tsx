...
export function AccountingOverviewPanel({ overview }: Props) {
  ...
  return (
    <div className="admin-accounting-overview-root">
      ...
      {(overview.withdrawalsCount > 0 || overview.withdrawalsYtd > 0) && (
        <section className="admin-accounting-section mb-8">
          <h2 className="admin-accounting-section-title">Personal withdrawals</h2>
          <p className="admin-hint mb-2 text-[0.85rem] leading-snug text-[var(--admin-text-muted,#6b7280)]">
            Money that left the business for personal use. Not counted as expenses — net profit is unchanged.
          </p>
          <div className={`${kpiShell} max-w-xl`}>
            <span className="material-symbols-outlined shrink-0 text-[1.5rem] text-violet-600 opacity-80" aria-hidden>
              savings
            </span>
            <div className="min-w-0">
              <p className={kpiLabel}>Personal withdrawals (this period)</p>
              <p className={kpiValue}>
                {fmt(overview.withdrawalsInPeriod)} · {overview.withdrawalsCount}{' '}
                {overview.withdrawalsCount === 1 ? 'withdrawal' : 'withdrawals'}
              </p>
              {overview.withdrawalsYtd > overview.withdrawalsInPeriod && (
                <p className={kpiSub}>Year to date: {fmt(overview.withdrawalsYtd)}</p>
              )}
              <p className="mt-2">
                <Link href={withdrawalsPeriodHref(sp)} className="admin-link text-[0.85rem] font-medium">
                  View all →
                </Link>
              </p>
            </div>
