...
export function CartPageClient({ lang }: { lang: Locale }) {
  ...
  // One-time preload: if the PDP's gift-details step already collected recipient
  // info (order-level draft), surface it here so the customer doesn't retype it.
  // Gates the local->context sync effect below until this completes, so that
  // effect doesn't fire first (with pre-seed local state) and clobber the draft.
  const [recipientSeededFromContext, setRecipientSeededFromContext] = useState(false);
  useEffect(() => {
    if (!cartHydrated || recipientSeededFromContext) return;
    const contextHasRecipient =
      orderRecipientName.trim().length > 0 || orderRecipientPhoneNational.trim().length > 0;
    if (contextHasRecipient) {
      setRecipientName(orderRecipientName);
      setRecipientCountryCode(orderRecipientCountryCode || '66');
      setRecipientPhoneNational(orderRecipientPhoneNational);
      setIsOrderingForSomeoneElse(true);
    }
    setRecipientSeededFromContext(true);
  }, [
    cartHydrated,
    recipientSeededFromContext,
    orderRecipientName,
    orderRecipientCountryCode,
    orderRecipientPhoneNational,
  ]);
  ...
  useEffect(() => {
    ...
    // Mirror into the order-level draft so the PDP's gift-details step stays in sync.
    // Gated on recipientSeededFromContext so this never overwrites the draft with
    // stale pre-seed local state on first mount (see the seed effect above).
    if (recipientSeededFromContext) {
      setOrderRecipientName(recipientName);
      setOrderRecipientCountryCode(recipientCountryCode);
      setOrderRecipientPhoneNational(recipientPhoneNational);
    }
  }, [items.length, delivery, customerName, customerEmail, countryCode, phoneNational, recipientName, recipientCountryCode, recipientPhoneNational, contactPreference, lineId, useLineIdFromPhone, isOrderingForSomeoneElse, surpriseDelivery, marketingEmailConsent, checkoutRecoveryEmailConsent, recipientSeededFromContext, setOrderRecipientName, setOrderRecipientCountryCode, setOrderRecipientPhoneNational]);
  ...
  return (
    <div className="cart-page cart-page--premium">
      ...
      <style jsx>{`
        ...
        .cart-expansion-block-notice {
          background: color-mix(in srgb, #fdba74 18%, var(--surface));
          border: 1px solid #fdba74;
          color: color-mix(in srgb, #9a3412 35%, var(--text));
        }
