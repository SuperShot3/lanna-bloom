...
function withdrawalMatchesPeriod(w: AccountingWithdrawal, f: LedgerPeriodFilter): boolean {
  const d = w.withdrawal_date.slice(0, 10);
  if (f.dateFrom && d < f.dateFrom.slice(0, 10)) return false;
  if (f.dateTo && d > f.dateTo.slice(0, 10)) return false;
  return true;
}

function toWithdrawalLedgerRow(
  w: AccountingWithdrawal,
  runningBalance: number,
  sortIso: string
): LedgerRow {
  const amt = Number(w.amount) || 0;
  const from = MONEY_LOC_LABEL[w.from_location] ?? w.from_location;
  const notes = w.notes ? ` · ${w.notes}` : '';
  return {
    id: w.id,
    kind: 'withdrawal',
    sortIso,
    displayDate: w.withdrawal_date.slice(0, 10),
    transactionType: 'withdrawal',
    category: 'Owner withdrawal',
    description: `Owner withdrawal — ${w.purpose}${notes}`,
    sourceAccount: from,
    amountIn: null,
    amountOut: amt,
    delta: 0,
...
function emptyLedgerResult(error?: string): LedgerResult {
  return {
    ...
    counts: { income: 0, expense: 0, transfer: 0, withdrawal: 0, missingReceipts: 0 },
    error,
  };
}
...
export async function getLedgerEntries(
  ...
  const withdrawals = (withdrawalResult.withdrawals ?? []) as AccountingWithdrawal[];
  ...
  type Raw = {
    ...
    withdrawal?: AccountingWithdrawal;
  };
  const raw: Raw[] = [];
  ...
  for (const w of withdrawals) {
    if (!hasPeriod || withdrawalMatchesPeriod(w, filter)) {
      const sortIso = w.created_at;
      raw.push({
        type: 'withdrawal',
  ...
  for (const item of raw) {
    if (item.type === 'income' && item.income) {
      ...
      rows.push(toWithdrawalLedgerRow(item.withdrawal, balance, item.sortIso));
    }
  }
  ...
  const counts = {
    ...
    withdrawal: rows.filter((r) => r.kind === 'withdrawal').length,
