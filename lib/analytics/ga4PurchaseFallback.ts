...
export async function tryProcessGa4PurchaseFallback(
  ...
  if (error || !row) return { status: 'skipped', reason: 'order_not_found' };
  ...
  const runAfterRaw = row.ga4_purchase_fallback_run_after;
  if (runAfterRaw) {
    const runAfterMs = new Date(runAfterRaw).getTime();
    if (Date.now() < runAfterMs) {
      return { status: 'skipped', reason: 'delay_not_elapsed' };
    }
  } else {
    await scheduleGa4PurchaseFallback(normalized);
    return { status: 'skipped', reason: 'scheduled' };
  }
