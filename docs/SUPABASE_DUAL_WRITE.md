...
4. Call `GET /api/admin/verify-supabase` with `x-admin-secret` header (or `Authorization: Bearer <ORDERS_ADMIN_SECRET>`); confirm both sources match
...
- `app/api/admin/verify-supabase/route.ts` — verification endpoint (admin-only)
