...
4. Call `GET /api/admin/verify-supabase` while logged into admin (session cookie); confirm both sources match
...
- `app/api/admin/verify-supabase/route.ts` — verification endpoint (admin session required)
