...
- **Purchase for revenue:** **GTM / dataLayer** — `trackPurchase` in `lib/analytics/gtag.ts`, called from `OrderPageClient` when `paid` is true. `sendPurchaseForOrder` (Measurement Protocol) is **deprecated** and not invoked by API routes.
