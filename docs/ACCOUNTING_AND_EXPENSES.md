...
|--------|--------------|---------|------------|
| Business expense (bank) | ↓ | unchanged | ↓ |
| Transfer Stripe → bank | unchanged overall | unchanged | unchanged |
| **Personal withdrawal** | ↓ | unchanged | unchanged |

Each withdrawal requires a **date**, **amount**, and **purpose** (where the money went). Optional notes and bank-slip proof. Withdrawals appear in the ledger for history but do **not** reduce the P&amp;L running balance (same idea as transfers).

**Do not use** expenses or cash transfers for personal spending — those affect profit or move money between business buckets.
...
| Area | Location |
|------|-----------|
| Overview aggregation | `lib/accounting/incomeRecords.ts` — `getAccountingOverview`; refund split in `lib/accounting/allocateOverviewRefunds.ts` |
| Refunds | `lib/accounting/incomeRefunds.ts`, `income_refunds` table; Accounting → Refunds |
